...
import { solanaBaseProvider, baseProvider } from "@/common/provider";
import { bn } from "@/common/utils";
import { getSolanaBlocks, saveSolanaBlock } from "@/models/blocks";
// import { Sources } from "@/models/sources";
// import { SourcesEntity } from "@/models/sources/sources-entity";
import { getTransaction, saveTransaction, saveSolanaTransactions } from "@/models/transactions";
...
const oneAddress = "11111111111111111111111111111111";

export const fetchBlock = async (blockNumber: number, force = false) =>
    getSolanaBlocks(blockNumber)
    // Only fetch a single block (multiple ones might be available due to reorgs)
    .then(async (blocks) => {
      if (blocks.length && !force) {
        return blocks[0];
      } else {
        const block = await solanaBaseProvider.getParsedBlock(blockNumber,{transactionDetails: "full", maxSupportedTransactionVersion:0 });
        // const block = await baseProvider.getBlockWithTransactions(blockNumber);
        
        // Create transactions array to store
        const transactions:any = block?.transactions.map((tx:any) => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const rawTx = tx.meta as any;

            const gasPrice = rawTx.fee?.toString();
            const gasUsed = rawTx.fee ? bn(rawTx.fee).toString() : undefined;
            const gasFee = gasPrice && gasUsed ? bn(gasPrice).mul(gasUsed).toString() : undefined;

            return {
              hash: tx.transaction.signatures[0].toLowerCase(),
              instructions: tx.transaction.message.instructions,
              // from: tx.from.toLowerCase(),
              // to: (tx.to || oneAddress).toLowerCase(),
              // value: "0", //tx.value.toString(),
        ...
        // await saveSolanaTransactions(transactions);

        console.log("Blog Addedd ------------------")
        
        console.log({ 
          number: blockNumber,
          hash: block.blockhash,
          timestamp: Number(block.blockTime)
        })

        return saveSolanaBlock({
          number: blockNumber,
