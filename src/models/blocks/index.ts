...
// For Solana network
export const saveSolanaBlock = async (block: Block): Promise<Block> => {
    await idb.none(
        `
        INSERT INTO blocks_solana (
            hash,
            number,
            "timestamp"
        ) VALUES (
...
export const deleteSolanaBlock = async (number: number, hash: string) =>
    idb.none(
        `
        DELETE FROM blocks
        WHERE blocks.hash = $/hash/
...
export const getSolanaBlocks = async (number: number): Promise<Block[]> =>
    idb
        .manyOrNone(
        `
            SELECT
